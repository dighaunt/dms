-- Modulo Finanzas — corte de caja diario (CACM-RCI-07), reparto de utilidades
-- y alertas.
--
-- El corte JALA, no recaptura. El unico dato nuevo que teclea el custodio es el
-- efectivo fisico contado; todo lo demas se arma leyendo los folios firmados de
-- esa fecha y esa sucursal. Por eso los totales del corte no son campos de
-- captura sino un snapshot que produce cerrar_corte_caja().
--
-- Dos criterios que conviene dejar escritos porque son interpretacion del
-- manual y no literalidad:
--
--  * Solo el efectivo mueve el arqueo. Un cobro con tarjeta o una liquidacion
--    que entra por transferencia son ingreso de la empresa, pero no engordan el
--    cajon; el catalogo forma_pago_fin.afecta_caja_fisica decide.
--  * La nomina sale de caja por su VALE (RCI-05), no por el recibo (RCI-06).
--    Contar ambos duplicaria el egreso. El recibo del trabajador queda citado
--    en el detalle del corte para poder rastrearlo, pero quien mueve el dinero
--    es el vale, en linea con la regla de que ningun egreso ocurre sin vale.
BEGIN;
...
-- Snapshot de que folios entraron al corte. Es lo que permite reconstruir el
-- corte anos despues aunque la consulta cambie.
...
-- Deposito bancario del dia: efectivo que sale del cajon hacia el banco. No es
-- un pago a un tercero, por eso no exige vale de egreso; lo que si exige es
-- comprobante, institucion y cuenta.
...
-- Incisos c) y d) del papel: en transito / por depositar, y otro resguardo.
...
-- ===== ALERTAS =====
-- Las alertas son hechos, no banderas mutables: se registran y se atienden.
...
-- El retiro de un socio es anticipo hasta que exista un balance que arroje
-- utilidades repartibles (LGSM art. 19). Este es ese respaldo formal, y sin el
-- ningun retiro puede presentarse como utilidad repartida en firme.
...
-- El snapshot del corte NO es append-only mientras el corte es un borrador:
-- armar_corte_caja lo rehace cada vez que el custodio agrega un deposito o
-- corrige algo, y esa es justamente la conducta deseada. Lo que no puede pasar
-- es que cambie DESPUES de cerrado. Por eso el guardia mira el estado del
-- corte, en lugar de prohibir toda mutacion.
...
BEGIN
    ...
    -- Un balance aprobado y su reparto no se reescriben nunca.
    ...
    -- La cabecera del corte sigue el mismo criterio que el resto de formatos:
    -- se puede corregir hasta que esta firmada.
...
-- ===== ARMADO Y CIERRE DEL CORTE =====

-- Saldo con el que abre el dia: el efectivo contado en el ultimo corte firmado
-- de esa sucursal. Encadenar los cortes evita que el saldo inicial sea un dato
-- que alguien pueda teclear a conveniencia.
...
-- Folios del dia que siguen sin firmar. Si hay alguno, el corte no puede
-- cerrarse: seria rendir cuentas de un dia cuyo dinero aun no tiene dueno.
...
-- Arma el corte leyendo los folios firmados del dia y guarda el snapshot.
-- No recibe importes: los calcula.
...
BEGIN
    ...
    -- Ingresos: ventas de contado (RCI-01, siempre efectivo por definicion del
    -- formato), utilidad neta de consignas (RCI-03) e ingresos de servicio
    -- (RCI-04); estos dos ultimos solo si entraron en efectivo.
    ...
    -- Egresos: vales de egreso pagados en efectivo, agrupados como los agrupa
    -- el papel en su Parte II.
    INSERT INTO corte_caja_detalle (corte_documento_id, origen_documento_id, naturaleza, concepto_grupo, importe)
    ...
    -- El deposito bancario del dia sale del cajon: es egreso de caja fisica.
...
-- Cierra el corte: arqueo, validaciones y paso a firma. El unico importe que
-- entra por parametro es el efectivo contado, tal como pide el manual.
...
BEGIN
    ...
    -- Sin esto, reintentar el cierre de un corte ya cerrado saldria por el
    -- mensaje interno de armar_corte_caja, que no le dice nada a quien opera.
...
-- ===== UBICACION DEL EFECTIVO AL CIERRE =====
-- Derivada, no capturada: caja fisica es el arqueo, banco son los depositos y
-- el resto son los resguardos declarados.
...
-- ===== ANTICIPOS DE SOCIOS =====
-- Lo retirado por cada socio frente a lo que un balance formal respalda. Un
-- saldo positivo es dinero por comprobar, no un gasto cerrado.
...
-- Un retiro de socio sin reparto formal vigente no se bloquea —la empresa
-- puede necesitar entregarlo— pero deja aviso para el gerente. Es exactamente
-- la diferencia entre un anticipo documentado y un reparto en firme.
...
-- ===== CUSTODIA PENDIENTE =====
-- Dinero que un vendedor declaro haber entregado y que ningun custodio ha
-- confirmado todavia. Mientras viva en esta vista, no es "dinero seguro en la
-- empresa": es dinero en transito bajo responsabilidad de quien lo entrego.
