...
export type PersonaEnPantalla = {
  ...
  notas: string | null;
  activa: boolean;
  /** Vales FIRMADOS que la citan: los únicos que movieron dinero. */
  vales: number;
  totalPagado: string;
...
/**
 * Las mismas cinco del CHECK de la columna. Es una PISTA para filtrar
 * selectores, no una verdad jurídica: quien es socio lo dice el registro de
 * socios y sólo él, y alguien puede estar aquí como PROVEEDOR y ser además
 * socio.
 */
const CATEGORIAS = [
...
/** Los mismos patrones que los CHECK de la tabla, para poder avisar antes. */
...
/** "" es "sin dato", no una cadena vacía que la base tenga que guardar. */
...
/**
 * Qué le falta al formulario para poder guardarse. Repite los CHECK de la
 * tabla para decirlo mientras se teclea, nunca para decidir en su lugar.
 */
                    ...
                    <TableRow key={persona.id} className={persona.activa ? undefined : "opacity-60"}>
                      ...
                      <TableCell className="text-sm text-muted-foreground">
                        {persona.idTipo && persona.idNumero ? (
                          ...
                          // No es un reproche: el catálogo la admite sin ella y
                          // el vale la exige de todos modos al momento de pagar.
                          "Sin registrar"
                        )}
                      </TableCell>
                      <TableCell className="text-sm text-muted-foreground">
...
/** Los mismos campos para el alta y para la corrección: uno solo, sin dos verdades. */
