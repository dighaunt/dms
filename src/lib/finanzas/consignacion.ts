...
/**
 * Inventario y consignación: CACM-RCI-02 (ingreso de vehículo) y CACM-RCI-03
 * (liquidación de la venta en consigna).
 *
 * Los candados de estos dos formatos ya están escritos en plpgsql (migración
 * 035) y aquí NO se repiten:
 *
 *  - `validar_ingreso_vehiculo_rci02` exige que el tipo de operación diga lo
 *    mismo que `expediente.origen` sobre la propiedad de la unidad;
 *  - `validar_liquidacion_rci03` sólo deja liquidar un ingreso que declaró
 *    consignación y que ya está FIRMADO;
 *  - `recalcular_gastos_rci03` mantiene `gastos_total`, y `utilidad_neta` es
 *    una columna GENERATED que nadie puede teclear;
 *  - `bloquear_detalle_documento_fin` congela el contenido tras la firma.
 *
 * Cuando uno se dispara llega como P0001 con su mensaje en español; se deja
 * propagar tal cual para que `respuestaError` de "@/lib/api" lo traduzca.
 *
 * Los importes viajan y se devuelven como cadena de principio a fin: las
 * columnas son numeric(18,2) y un double pierde centavos. La aritmética que
 * este módulo necesita se hace en centavos enteros con `aCentavos`/`deCentavos`.
 */

// ===== VOCABULARIO DEL RCI-02 =====

/**
 * Los dos modos en que una unidad entra al piso. Es copia del CHECK de
 * `ingreso_vehiculo_rci02.tipo_operacion`; el catálogo llega a la pantalla por
 * la ruta de API que envuelve a este servicio, nunca importando este módulo,
 * que es `server-only`.
 */
...
/**
 * Correspondencia que impone el disparador entre el origen del expediente y el
 * tipo de operación del RCI-02. Se usa para PRESELECCIONAR la opción en
 * pantalla, no para decidir: si el expediente estuviera mal, quien captura debe
 * ver el rechazo del disparador y corregir el expediente, que es la única
 * verdad sobre si la unidad es propia o de un tercero. Derivar el valor en
 * silencio convertiría el error en dato bueno.
 */
 ...
 * entregó al comprador, se devolvió al consignante o se dio de baja. Dejan de
 * aparecer en el listado porque ya no hay nada físico que custodiar.
 */
const ESTADOS_UNIDAD_FUERA_DE_RESGUARDO = ["ENTREGADA", "DEVUELTA_CONSIGNANTE", "BAJA"];

/** Longitud mínima que el CHECK de `ajuste_utilidad_rci03` exige a la nota. */
export const MINIMO_NOTA_AUDITORIA_AJUSTE = 20;

// ===== ESQUEMAS =====

/** Un código de catálogo administrable (forma de pago). Quien lo valida de
 * verdad es la llave foránea contra `forma_pago_fin`: el manual encarga esos
 * catálogos al administrador y congelarlos aquí obligaría a un despliegue para
 * dar de alta una forma de pago nueva. */
...
/**
 * Porcentaje de comisión pactada. Va como cadena por la misma razón que el
 * dinero: la columna es numeric(5,2) y "12.34" tecleado como double ya no
 * vuelve a ser 12.34. Se compara en centésimas enteras.
 */
...
/** Parte I y II del papel: la unidad que entra y quien la entrega. */
const esquemaIngresoComun = z.object({
  /** El RCI-02 va anclado a un expediente ya abierto: de ahí salen VIN y ficha. */
  expedienteId: esquemaId,
  placas: z.string().trim().max(20).nullish(),
  // El tope es el del dato maestro de la unidad: el mismo límite en el wizard
  // del expediente y en el formato financiero, o una pantalla aceptaría un
  // odómetro que la otra rechaza.
...
/**
 * Parte III: las condiciones económicas, que son excluyentes según el tipo de
 * operación. La unión discriminada describe QUÉ campos existen en cada caso, de
 * modo que un precio de compra enviado en una consigna ni siquiera llegue a la
 * consulta. Que la comisión se pacte como monto O como porcentaje —nunca ambos
 * ni ninguno— lo impone el CHECK de la tabla y no se repite aquí: dos versiones
 * de la misma regla acaban separándose.
 */
...
export const esquemaLiquidacion = z.object({
  /** Folio del RCI-02 que declaró la consignación; debe estar firmado. */
  ingresoRci02Id: esquemaId,
  /** Recibo de caja con el que entró el dinero de la venta, si lo hubo. */
  reciboRci01Id: esquemaId.nullish(),
  /** Documento del expediente que ampara la venta (C-02, C-04…). */
  ...
  /**
   * Renglones de gasto. Si se omiten, los ya capturados se conservan; si vienen
   * —aunque sea vacío— sustituyen a los anteriores.
   */
...
export const esquemaPrevisualizacionUtilidad = z.object({
  /** Cuando se indica, se contrasta contra lo pactado en el RCI-02. */
...
export type DatosPrecargadosExpediente = {
  ...
  /** PROPIA o CONSIGNADA: es la verdad sobre de quién es la unidad. */
  origen: OrigenUnidad;
  /** Opción que corresponde a ese origen, para preseleccionarla en pantalla. */
  ...
  /** Odómetro al día 0 del expediente; el RCI-02 vuelve a pedirlo. */
  kilometrajeIngreso: number | null;
  /** Verdadero si esta unidad ya tiene su RCI-02: no se emite un segundo. */
...
export type LiquidacionConsigna = {
  ...
  /** Suma de los renglones de gasto; la mantiene un disparador. */
  gastosTotal: string;
  /** Columna GENERATED: precio − consignante − gastos. Es la cifra que se firma. */
  utilidadNeta: string;
  /**
   * `utilidadNeta` más los ajustes posteriores. Sirve para reportes; lo que el
   * consignante firmó sigue siendo `utilidadNeta`.
   */
...
export type PrevisualizacionUtilidad = {
  ...
  montoConsignante: string;
  gastosTotal: string;
  /** Espejo del cálculo, NO la cifra buena. Ver `previsualizarUtilidad`. */
  utilidadNeta: string;
  esNegativa: boolean;
  /** Lo pactado en el RCI-02, cuando se indicó cuál es. */
  precioMinimoPactado: string | null;
  comisionPactadaMonto: string | null;
  comisionPactadaPct: string | null;
  /** Aviso, no candado: el consignante puede aceptar menos, pero hay que verlo. */
...
export type UnidadEnConsignacion = {
  ...
  comisionPct: string | null;
  consignaFechaLimite: string | null;
  /** Verdadero sólo con el RCI-03 firmado: liquidada de verdad. */
  liquidada: boolean;
  liquidacionDocumentoId: number | null;
  /** Siempre falso. Existe para que ningún reporte tenga que deducirlo. */
...
/** Los `timestamptz` llegan como Date por el driver; hacia afuera van en ISO. */
function aIso(valor: Date | string): string {
  return (valor instanceof Date ? valor : new Date(valor)).toISOString();
}

/** Suma centavos de una lista de importes ya canónicos. */
function sumarImportes(importes: string[]): bigint {
  return importes.reduce((total, importe) => total + (aCentavos(importe) ?? 0n), 0n);
}

// ===== DATOS PRECARGADOS DEL EXPEDIENTE =====

/**
 * Ficha de la unidad para prellenar el RCI-02.
 *
 * El formato de papel vuelve a pedir marca, modelo, año y VIN, pero esos datos
 * ya están en el expediente: volver a teclearlos es la manera más fácil de que
 * el folio financiero y el expediente acaben describiendo dos coches distintos.
 * Se leen de `traza.unidad` y se devuelven para mostrarlos, no para que la
 * pantalla los reenvíe: la consulta del RCI-02 nunca los guarda de nuevo.
 */
...
// ===== CACM-RCI-02 — INGRESO DE VEHÍCULO =====
...
/**
 * Los identificadores se piden como `::int` y los importes y fechas como
 * `::text` en la propia consulta: así el driver entrega números donde el tipo
 * dice número y cadenas donde hay dinero o fecha civil, sin que una conversión
 * en JavaScript pueda perder centavos ni correr un día por el huso horario.
 */
...
/**
 * Guarda el contenido del RCI-02 sobre un folio ya emitido.
 *
 * Es un alta-o-reemplazo porque quien captura guarda varias veces mientras
 * revisa el papel; en cuanto el folio se firma, el disparador
 * `bloquear_detalle_documento_fin` rechaza el siguiente intento y la corrección
 * pasa a ser un complementario. La coincidencia entre `tipoOperacion` y
 * `expediente.origen` no se comprueba aquí: la impone el disparador
 * `validar_ingreso_vehiculo_rci02`, cuyo mensaje dice además cuál de los dos
 * hay que corregir.
 */
export async function capturarIngresoVehiculo(
  ...
  // La tabla no guarda quién capturó —la autoría del folio vive en
  // documento_financiero.creado_por y en las firmas—, pero el id se exige y se
  // valida igual: obliga a que toda ruta lo haya resuelto de la sesión
  // (src/lib/auth/usuario.ts) en vez de aceptarlo del cuerpo de la petición.
...
/** Contenido del RCI-02 con la ficha de la unidad ya resuelta. */
...
// ===== CACM-RCI-03 — LIQUIDACIÓN DE LA CONSIGNA =====
...
/**
 * Lee la liquidación completa. Va dentro de una transacción porque cabecera,
 * gastos y ajustes tienen que verse en el mismo instante: si un gasto entrara
 * entre dos consultas, `gastosTotal` no cuadraría con los renglones mostrados.
 */
...
/**
 * Sustituye el renglonaje de gastos completo.
 *
 * Se borra y se vuelve a escribir en lugar de ir renglón por renglón porque la
 * pantalla manda la lista tal como quedó; el disparador
 * `recalcular_gastos_rci03` rehace `gastos_total` con cada movimiento, así que
 * la utilidad generada siempre corresponde a los renglones que de verdad
 * existen. `gastos_total` jamás se escribe desde aquí.
 */
async function reemplazarGastos(
  ...
  await cliente.query(
    ...
    [
      ...
      // Los importes viajan como texto hasta el último momento y es Postgres
      // quien los convierte a numeric(18,2): un round trip por double redondearía.
...
/**
 * Guarda el contenido del RCI-03 sobre un folio ya emitido.
 *
 * Que el ingreso citado sea una consignación y esté FIRMADO lo comprueba el
 * disparador `validar_liquidacion_rci03`: liquidar contra un borrador sería
 * repartir dinero bajo condiciones que el consignante todavía no aceptó.
 * `gastos_total` y `utilidad_neta` no aparecen en la consulta a propósito: la
 * primera la mantiene un disparador y la segunda es GENERATED.
 */
export async function capturarLiquidacion(
  ...
  // Igual que en el RCI-02: el id de sesión se exige aunque la tabla no lo
  // guarde, para que ninguna ruta lo tome del cuerpo de la petición.
  esquemaId.parse(usuario);
  ...
  return withTransaction(async (cliente) => {
    ...
    // La cabecera acaba de escribirse, así que la fila existe con seguridad.
    return (await leerLiquidacion(cliente, id)) as LiquidacionConsigna;
  });
}

/** Liquidación completa: cabecera, gastos y ajustes en una misma foto. */
...
/**
 * Captura los gastos de la liquidación. La lista recibida sustituye a la
 * anterior; devolver la liquidación ya recalculada evita que la pantalla tenga
 * que sumar por su cuenta una utilidad que sólo la base puede dar por buena.
 */
export async function registrarGastos(
  ...
  return withTransaction(async (cliente) => {
    ...
    if (!liquidacion) {
      // Sin cabecera no hay a qué colgar los gastos; la llave foránea ya habría
      // rechazado el INSERT, pero una lista vacía no dispara ninguna escritura.
 ...
 * es una columna GENERATED a partir del precio, lo del consignante y los
 * gastos, de modo que la cifra siempre sea reproducible desde sus tres
 * sumandos. Mover el resultado a mano rompería esa reproducibilidad y, peor,
 * cambiaría en silencio el número que el consignante firmó —el contenido queda
 * congelado tras la firma justamente para eso—. Registrarlo aparte convierte
 * una corrección invisible en un hecho con autor, fecha y explicación, que
 * además es inmutable (`ajuste_utilidad_rci03_inmutable`). La nota de auditoría
 * es lo único que justifica el movimiento; por eso la base le exige 20
 * caracteres y aquí se comprueba antes de viajar.
 */
...
/**
 * Calcula la utilidad para MOSTRARLA antes de guardar.
 *
 * Es un espejo, no la cifra buena: la que vale es `utilidad_neta`, columna
 * GENERATED de `liquidacion_consigna_rci03`, calculada por la base sobre los
 * renglones de gasto realmente guardados. Ésta se produce con
 * `utilidadConsigna` —la misma resta, en centavos enteros— para que quien
 * captura vea el resultado mientras teclea; si las dos difirieran, manda la
 * base y esta pantalla está mostrando datos que aún no existen.
 *
 * Cuando se indica el RCI-02 se traen además las condiciones pactadas, porque
 * liquidar por debajo del mínimo acordado es lo que un socio necesita ver antes
 * de firmar. Es un aviso y no un candado: la base no lo prohíbe, y no debe,
 * porque un consignante puede aceptar menos.
 */
export async function previsualizarUtilidad(
  ...
  if (!calculo) {
    // Inalcanzable: los esquemas ya canonizaron cada importe. Si ocurriera,
    // es un defecto del código y no una regla de negocio incumplida.
...
/**
 * Unidades consignadas que siguen bajo nuestro resguardo.
 *
 * Están físicamente en el piso y por eso un conteo las ve, pero la propiedad
 * nunca se transmitió: sumarlas al inventario propio inflaría el activo de la
 * empresa con coches ajenos y daría por ganado un dinero que en realidad se le
 * debe al consignante. De ahí que cada renglón salga ya etiquetado y con
 * `cuentaEnInventarioPropio` en falso: ningún reporte debería tener que
 * deducirlo por su cuenta, y quien escriba uno nuevo se topa con la bandera
 * antes que con la duda.
 *
 * El origen del expediente es la única fuente de esa verdad —el RCI-02 puede
 * no existir todavía—, así que el filtro va por `expediente.origen` y no por el
 * folio financiero. Se excluyen las que ya salieron (entregadas, devueltas al
 * consignante o dadas de baja) porque de ésas no queda nada que custodiar.
 */
export async function unidadesEnConsignacion(): Promise<UnidadEnConsignacion[]> {
  ...
  return rows.map((fila) => ({
    ...
    comisionPct: fila.comision_pct,
    consignaFechaLimite: fila.consigna_fecha_limite,
    // Sólo el RCI-03 firmado liquida: un borrador o un folio cancelado no le
    // entregaron un peso a nadie.
